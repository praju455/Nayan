...
| --- | --- | --- |
| Text PII | [Gretel Synthetic PII Finance Multilingual](https://huggingface.co/datasets/gretelai/synthetic_pii_finance_multilingual) | Evaluate PII span detection and placeholder masking using Apache-2.0 synthetic data. |
...
| Screenshot UI grounding | [GroundCUA](https://github.com/ServiceNow/GroundCUA) | Evaluate locating labelled UI targets in a screenshot. |
| End-to-end browser tasks | [OSWorld](https://github.com/xlang-ai/OSWorld) | Evaluate perceive → sanitize → plan → act workflows. |

These datasets are needed for the next validation phase; the current synthetic fixtures are not enough to claim real-world accuracy. Start with 2,000 held-out Gretel PII samples, 500 WIDER FACE validation images, 300 GroundCUA screenshot/target pairs, and 50–100 controlled real-web captures from test accounts. Keep raw captures local; commit only sanitized examples, annotations, and aggregate metrics. Check each dataset licence before use or redistribution.

The PII benchmark uses Gretel's Apache-2.0 synthetic finance dataset, so it does not require a separate permission request. The generated fixture is still local-only and ignored by Git.

```bash
npm run benchmark:pii
npm run evaluate:pii
npm run benchmark:status
```

This creates a deterministic, local-only 2,000-sample held-out fixture from the published test split with verified PII spans. `npm run evaluate:pii` measures character-span precision, recall, and F1 for label types handled by Nayan's current regex recognizer. It never sends the dataset to the planner or commits it to the repository.
...
GroundCUA provides labelled desktop screenshots and UI bounding boxes. It is for research/educational use, so review its terms first. The importer samples 30 applications with 10 labelled screenshot targets each, then keeps the 300 screenshots, source annotations, and resulting manifest local and ignored by Git:

```bash
NAYAN_GROUNDCUA_RESEARCH_USE_CONFIRMED=1 npm run benchmark:groundcua
npm run benchmark:status
```
