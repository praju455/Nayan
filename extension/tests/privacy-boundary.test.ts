...
describe("privacy boundary", () => {
  ...
  it("keeps exact draft text in the local vault even when it is not conventional PII", () => { const vault = new TokenVault(); const task = sanitizeTask("Draft a message but do not send it.", vault, "hello from Nayan"); expect(task).toContain("<USER_PROVIDED_TEXT_"); expect(task).not.toContain("hello from Nayan"); });
});
