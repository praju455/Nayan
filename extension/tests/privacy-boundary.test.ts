...
const safePayload: SanitizedContextPackage = {
  protocolVersion: "1.0", taskId: "task_demo", screen: { width: 1280, height: 720 }, task: "Enter <EMAIL_1_x7> in the email field.", tabs: [],
...
describe("privacy boundary", () => {
  ...
  it("extracts an in-task chat recipient and quoted draft into local tokens", () => { const vault = new TokenVault(); const task = sanitizeTask('Open the chat named dii_yeahhh and type "hi".', vault); expect(task).toContain("<USER_SELECTED_RECIPIENT_"); expect(task).toContain("<USER_PROVIDED_TEXT_"); expect(task).not.toContain("dii_yeahhh"); expect(task).not.toContain('"hi"'); });
});
