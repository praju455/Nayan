...
export type PiiCategory = "EMAIL" | "PHONE" | "CREDIT_CARD" | "PAN" | "AADHAAR" | "IP_ADDRESS" | "DATE_OF_BIRTH" | "BANK_ACCOUNT" | "PASSWORD" | "EMPLOYEE_ID" | "PERSON_NAME" | "ADDRESS" | "FACE";
...
export type SanitizedContextPackage = Readonly<{ protocolVersion: "1.0"; taskId: string; screen: { width: number; height: number }; task: string; elements: readonly SanitizedElement[]; redactions: readonly RedactionRecord[]; state: { step: number; pageFingerprint: string }; redactedScreenshot?: string | null }>;
export type AgentAction = Readonly<{ action: "click" | "type" | "scroll" | "select" | "focus" | "navigate" | "wait" | "done" | "confirm_needed"; targetId?: string; valueToken?: string; destination?: string; deltaY?: number; message?: string; confidence: number; reason: string }>;
