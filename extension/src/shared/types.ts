...
export type SanitizedTab = Readonly<{ id: number; title?: string; origin: string; active: boolean }>;
...
export type SanitizedContextPackage = Readonly<{ protocolVersion: "1.0"; taskId: string; screen: { width: number; height: number }; task: string; tabs: readonly SanitizedTab[]; elements: readonly SanitizedElement[]; redactions: readonly RedactionRecord[]; state: { step: number; pageFingerprint: string; confirmed?: boolean }; redactedScreenshot?: string | null }>;
export type AgentAction = Readonly<{ action: "click" | "type" | "scroll" | "select" | "click_visible_text" | "activate_tab" | "focus" | "navigate" | "wait" | "done" | "confirm_needed"; targetId?: string; tabId?: number; valueToken?: string; destination?: string; deltaY?: number; message?: string; confidence: number; reason: string }>;
