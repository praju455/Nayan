...
const allowed = new Set<AgentAction["action"]>(["click", "type", "scroll", "select", "focus", "navigate", "wait", "done", "confirm_needed"]);
...
export function validateLocalAction(action: AgentAction, elements: readonly SanitizedElement[], tokenExists: (token: string) => boolean): string | undefined {
  ...
  if (targetActions.has(action.action)) { const target = elements.find((element) => element.id === action.targetId); if (!target || !target.visible || !target.interactive) return "Target is stale, hidden, or non-interactive"; if (action.action === "type" && !action.valueToken) return "Typing requires a local token"; }
