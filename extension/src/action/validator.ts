...
export function validateLocalAction(action: AgentAction, elements: readonly SanitizedElement[], tokenExists: (token: string) => boolean): string | undefined {
  ...
  if ((action.action === "type" || action.action === "select" || action.action === "click_visible_text") && !action.valueToken) return "Changing a page field or selecting visible text requires a local token";
