...
export class NayanAgent {
  ...
  private async runStep(confirmed: boolean): Promise<AgentRunResult> {
    ...
    const artifact = createSanitizedOutput({ rawFrame, taskId: `task_${crypto.randomUUID()}`, task: sanitizeTask(task, this.vault), elements, redactions: sanitized.redactions, step: this.step++, pageFingerprint: await this.fingerprint(nodes), confirmed });
    ...
    if (action.action === "confirm_needed") return { status: "confirmation_required", action, redactionCount: sanitized.redactions.length, modelRuntime: this.perception.runtime, message: action.message };
    if (action.action === "done") { this.vault.clear(); this.active = undefined; return { status: "done", action, redactionCount: sanitized.redactions.length, modelRuntime: this.perception.runtime }; }
    const invalid = validateLocalAction(action, elements, (token) => this.vault.has(token));
    if (invalid) return { status: "blocked", action, redactionCount: sanitized.redactions.length, modelRuntime: this.perception.runtime, message: invalid };
    ...
    if (!outcome.ok) return { status: "blocked", action, redactionCount: sanitized.redactions.length, modelRuntime: this.perception.runtime, message: outcome.reason };
    if (this.step < 10) { await new Promise<void>((resolve) => setTimeout(resolve, 180)); return this.runStep(false); }
    return { status: "blocked", action, redactionCount: sanitized.redactions.length, modelRuntime: this.perception.runtime, message: "Stopped after the maximum safe step count." };
  }
