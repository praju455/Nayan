...
allowSite.addEventListener("click", () => void (async () => { try { const result = await browser.runtime.sendMessage({ type: "NAYAN_ALLOW_CURRENT_SITE", task: task.value }) as { origin: string }; status.textContent = `Approved ${result.origin}. You can now start Nayan on this site.`; } catch (error) { status.textContent = error instanceof Error ? error.message : "Nayan could not approve this site."; } })());
