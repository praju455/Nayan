...
function show(result: Result): void { status.textContent = `${result.status.replaceAll("_", " ")} · ${result.redactionCount} local redactions · ${result.modelRuntime.toUpperCase()} mode${result.message ? ` · ${result.message}` : ""}`; payload.textContent = JSON.stringify(result.safeContext, null, 2); preview.hidden = !result.localPreviewDataUrl; if (result.localPreviewDataUrl) preview.src = result.localPreviewDataUrl; confirm.hidden = result.status !== "confirmation_required"; confirm.textContent = /send/i.test(`${result.message ?? ""} ${result.action.reason}`) ? "Confirm send" : "Confirm and submit"; start.disabled = result.status === "confirmation_required"; }
async function send(type: "NAYAN_START" | "NAYAN_CONFIRM"): Promise<void> { try { const result = await browser.runtime.sendMessage(type === "NAYAN_START" ? { type, task: task.value, recipient: recipient.value, draftText: draftText.value, autoSubmitDemo: autoDemo.checked } : { type }) as Result; show(result); } catch (error) { status.textContent = error instanceof Error ? error.message : "Nayan could not safely continue."; } }
