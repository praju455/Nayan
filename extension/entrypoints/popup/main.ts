...
type Result = { status: "confirmation_required" | "done" | "acted" | "blocked"; action: AgentAction; redactionCount: number; modelRuntime: string; message?: string };
...
function show(result: Result): void { status.textContent = `${result.status.replaceAll("_", " ")} · ${result.redactionCount} local redactions · ${result.modelRuntime.toUpperCase()} mode${result.message ? ` · ${result.message}` : ""}`; confirm.hidden = result.status !== "confirmation_required"; start.disabled = result.status === "confirmation_required"; }
