...
export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener(async (message: { type?: string; task?: string; serverUrl?: string; autoSubmitDemo?: boolean; draftText?: string }) => {
    ...
    if (message.type === "NAYAN_START") {
      ...
      return agent.start(tab.id, message.task, message.serverUrl || "http://localhost:8000", message.autoSubmitDemo === true && isLocalDemo, message.draftText);
    }
