...
1. Use a separate Gmail account containing only non-sensitive test mail. Start the FastAPI server with `NAYAN_REASONING_BACKEND=groq` and a Groq key for an open-weight model such as a Llama or Qwen family model.
...
- Groq receives only the sanitized reasoning context and returns one constrained, locally validated action. Raw page data never leaves the extension.
